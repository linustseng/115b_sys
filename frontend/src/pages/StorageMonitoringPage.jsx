  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-7 sm:px-10"><div className="mx-auto max-w-6xl">
    ...
    {snapshot ? <>
      <section className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <article className="rounded-3xl bg-white p-5 shadow-sm"><p className="text-xs font-semibold text-slate-500">實際已用</p><p className="mt-2 text-2xl font-bold text-slate-900">{formatGiB(snapshot.actualUsedBytes)}</p><p className="mt-1 text-xs text-slate-500">{Number(snapshot.actualUsedBytes || 0).toLocaleString()} bytes</p></article>
        <article className="rounded-3xl bg-white p-5 shadow-sm"><p className="text-xs font-semibold text-slate-500">可用配額 / 方案</p><p className="mt-2 text-xl font-bold text-slate-900">{quotaAvailable ? formatGiB(snapshot.quota.bytes) : "未設定"}</p><p className="mt-1 text-xs text-slate-500">{quotaAvailable ? snapshot.quota.planLabel || "已設定專案配額" : "不從方案名稱猜測組織配額"}</p></article>
        <article className="rounded-3xl bg-white p-5 shadow-sm"><p className="text-xs font-semibold text-slate-500">剩餘</p><p className="mt-2 text-2xl font-bold text-slate-900">{quotaAvailable ? formatGiB(snapshot.remainingBytes) : "—"}</p><p className="mt-1 text-xs text-slate-500">{quotaAvailable ? "以已設定專案配額計算" : "需要經核准的配額設定"}</p></article>
