...
export default function ActivityAlbumsPage({ shared }) {
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-7 sm:px-10"><div className="mx-auto max-w-6xl">
    ...
    <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">{visible.map((album) => <button key={album.id} onClick={() => openAlbum(album)} className="overflow-hidden rounded-xl border border-slate-200 bg-white text-left transition hover:border-cyan-300 hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500"><div className="aspect-[4/3] bg-slate-100">{album.coverUrl && <img src={album.coverUrl} alt="" className="h-full w-full object-cover" />}</div><div className="p-3"><h2 className="truncate text-sm font-semibold text-slate-900">{album.title}</h2><p className="mt-0.5 truncate text-xs text-slate-500">{album.eventDate?.replaceAll("-", "/") || "未設定日期"} · {album.photoCount} 張</p></div></button>)}</div>
