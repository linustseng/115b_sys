...
export default function ActivityAlbumsPage({ shared }) {
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-7 sm:px-10"><div className="mx-auto max-w-6xl">
    ...
    <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{visible.map((album) => <button key={album.id} onClick={() => openAlbum(album)} className="overflow-hidden rounded-3xl bg-white text-left shadow-sm"><div className="aspect-[4/3] bg-gradient-to-br from-cyan-100 to-indigo-100">{album.coverUrl && <img src={album.coverUrl} alt="" className="h-full w-full object-cover" />}</div><div className="p-5"><h2 className="font-semibold text-slate-900">{album.title}</h2><p className="mt-1 text-sm text-slate-500">{album.eventDate?.replaceAll("-", "/") || "未設定日期"} · {album.photoCount} 張</p></div></button>)}</div>
