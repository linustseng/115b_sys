...
- `VITE_API_V2_READ_ENABLED`: Enable read actions (`listEvents`, `listHomeBootstrap`) to try Node API first and fallback to Apps Script (optional, set `1`)
- `VITE_API_V2_TIMEOUT_MS`: Timeout for Node API read path (optional, default same as `VITE_API_TIMEOUT_MS`)
