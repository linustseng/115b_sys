...
- `VITE_API_V2_READ_ENABLED`: Enable selected read actions (`listEvents`, `listHomeBootstrap`, `listStudents`, `listGroupMemberships`, `lookupStudent`, `listMyMemberships`, `getRegistrationBootstrap`, `getCheckinBootstrap`, `listCheckinStatus`) to try Node API first and fallback to Apps Script (optional, set `1`)
- `VITE_API_V2_WRITE_ENABLED`: Enable selected write actions (`register`, `checkin`, `updateRegistration`) to go through Node API first and fallback to Apps Script on transport failure (optional, set `1`)
