...
function isActivityPhotoMime_(mime) {
  return new Set(["image/jpeg", "image/jpg", "image/png", "image/heic", "image/heif"]).has(String(mime || "").trim().toLowerCase());
}
...
async function getActivityAlbum_(albumId) {
  const result = await query(`select * from activity_albums where id = $1 limit 1`, [albumId]);
  return result.rows[0] || null;
}

async function hydrateActivityAlbum_(album) {
  if (!album) return null;
  let coverUrl = "";
  if (album.cover_photo_id) {
    const cover = await query(`select * from activity_photos where id = $1 and album_id = $2 and status = 'ready' limit 1`, [album.cover_photo_id, album.id]);
    if (cover.rows[0]) coverUrl = await createSignedReadUrlForAttachment(cover.rows[0], 900);
  }
  return { ...album, cover_url: coverUrl };
}
...
app.get("/v1/activity-albums", async (req, res) => {
  try {
    const auth = await resolveAuthContext(req);
    if (!auth || !auth.studentId) return res.status(401).json({ ok: false, data: null, error: "Unauthorized" });
    const canManage = await canManageActivityAlbums_(auth.studentId);
    const includeArchived = canManage && String(req.query && req.query.includeArchived || "") === "1";
    const result = await query(`select a.*, count(p.id)::int as photo_count
      from activity_albums a left join activity_photos p on p.album_id = a.id and p.status = 'ready'
      where ($1::boolean or a.status <> 'archived') group by a.id order by a.event_date desc nulls last, a.created_at desc`, [includeArchived]);
    const albums = await Promise.all(result.rows.map(hydrateActivityAlbum_));
    return res.json({ ok: true, data: { albums, canManage }, error: null });
  } catch (error) { return res.status(500).json({ ok: false, data: null, error: error.message || "Internal error" }); }
});
...
app.post("/v1/activity-albums", async (req, res) => {
  try {
    const auth = await resolveAuthContext(req);
    if (!auth || !auth.studentId) return res.status(401).json({ ok: false, data: null, error: "Unauthorized" });
    if (!(await canManageActivityAlbums_(auth.studentId))) return res.status(403).json({ ok: false, data: null, error: "Forbidden" });
    const title = firstText(req.body && req.body.title).slice(0, 120);
    const eventDate = firstText(req.body && req.body.eventDate);
    if (!title) return res.status(400).json({ ok: false, data: null, error: "Missing album title" });
    if (!isIsoDate_(eventDate)) return res.status(400).json({ ok: false, data: null, error: "Invalid event date" });
    const id = crypto.randomUUID();
    const result = await query(`insert into activity_albums (id,title,description,event_date,location,created_by)
      values ($1,$2,$3,$4,$5,$6) returning *`, [id, title, firstText(req.body && req.body.description).slice(0, 1000), eventDate || null, firstText(req.body && req.body.location).slice(0, 160), auth.studentId]);
    return res.json({ ok: true, data: { album: result.rows[0] }, error: null });
  } catch (error) { return res.status(500).json({ ok: false, data: null, error: error.message || "Internal error" }); }
});
...
app.patch("/v1/activity-albums/:id", async (req, res) => {
  try {
    ...
    const existing = await getActivityAlbum_(albumId);
    if (!existing) return res.status(404).json({ ok: false, data: null, error: "Album not found" });
    const body = req.body && typeof req.body === "object" ? req.body : {};
    const title = Object.hasOwn(body, "title") ? firstText(body.title).slice(0, 120) : existing.title;
    const description = Object.hasOwn(body, "description") ? firstText(body.description).slice(0, 1000) : existing.description;
    const eventDate = Object.hasOwn(body, "eventDate") ? firstText(body.eventDate) : firstText(existing.event_date);
    const location = Object.hasOwn(body, "location") ? firstText(body.location).slice(0, 160) : existing.location;
    ...
    if (!title || !isIsoDate_(eventDate) || !["active", "archived"].includes(status)) return res.status(400).json({ ok: false, data: null, error: "Invalid album settings" });
    if (coverPhotoId) {
      const cover = await query(`select id from activity_photos where id = $1 and album_id = $2 and status = 'ready' limit 1`, [coverPhotoId, albumId]);
      if (!cover.rows[0]) return res.status(400).json({ ok: false, data: null, error: "Cover photo must be a visible photo in this album" });
    }
    const result = await query(`update activity_albums
      set title = $2, description = $3, event_date = $4, location = $5, status = $6, cover_photo_id = $7, updated_at = now()
      where id = $1 returning *`, [albumId, title, description, eventDate || null, location, status, coverPhotoId || null]);
...
app.get("/v1/activity-albums/:id/photos", async (req, res) => {
  try {
    ...
    if (!auth || !auth.studentId) return res.status(401).json({ ok: false, data: null, error: "Unauthorized" });
    const albumId = String(req.params.id || "").trim();
    const album = await getActivityAlbum_(albumId);
    if (!album) return res.status(404).json({ ok: false, data: null, error: "Album not found" });
    const canManage = await canManageActivityAlbums_(auth.studentId);
    if (album.status === "archived" && !canManage) return res.status(404).json({ ok: false, data: null, error: "Album not found" });
    const includeHidden = canManage && String(req.query && req.query.includeHidden || "") === "1";
    const result = await query(`select * from activity_photos where album_id = $1 and (status = 'ready' or ($2::boolean and status = 'hidden')) order by captured_at desc nulls last, created_at desc`, [albumId, includeHidden]);
    const photos = await Promise.all(result.rows.map(async (row) => ({ ...row, signed_url: await createSignedReadUrlForAttachment(row, 900) })));
    return res.json({ ok: true, data: { album: await hydrateActivityAlbum_(album), photos, canManage }, error: null });
  } catch (error) { return res.status(500).json({ ok: false, data: null, error: error.message || "Internal error" }); }
});
...
app.post("/v1/activity-albums/:id/upload-intent", async (req, res) => {
  try {
    const auth = await resolveAuthContext(req);
    if (!auth || !auth.studentId) return res.status(401).json({ ok: false, data: null, error: "Unauthorized" });
    if (!isAttachmentStorageConfigured()) return res.status(500).json({ ok: false, data: null, error: "Supabase Storage 尚未設定" });
    const albumId = String(req.params.id || "").trim();
    const fileName = safeFilename_(req.body && req.body.fileName);
    const mimeType = String(req.body && req.body.mimeType || "").trim().toLowerCase();
    const sizeBytes = Number(req.body && req.body.sizeBytes || 0);
    const album = await getActivityAlbum_(albumId);
    if (!album || album.status !== "active") return res.status(404).json({ ok: false, data: null, error: "Album not found" });
    if (!isActivityPhotoMime_(mimeType) || !fileName || !Number.isFinite(sizeBytes) || sizeBytes <= 0 || sizeBytes > 15 * 1024 * 1024) return res.status(400).json({ ok: false, data: null, error: "照片格式或大小不符合規範" });
    const photoId = crypto.randomUUID();
    const storagePath = `activity-albums/${albumId}/${photoId}-${fileName}`;
    const upload = await createSignedUploadUrl({ storagePath, mimeType });
    const profile = await findStudentProfileById(auth.studentId);
    await query(`insert into activity_photos (id,album_id,bucket,storage_path,original_name,mime_type,size_bytes,uploaded_by,uploaded_by_name)
      values ($1,$2,$3,$4,$5,$6,$7,$8,$9)`, [photoId, albumId, upload.bucket, storagePath, fileName, mimeType, sizeBytes, auth.studentId, firstText(profile && (profile.name || profile.nameZh), auth.profile && auth.profile.name)]);
    return res.json({ ok: true, data: { photoId, signedUrl: upload.signedUrl, token: upload.token, storagePath }, error: null });
  } catch (error) { return res.status(500).json({ ok: false, data: null, error: error.message || "Internal error" }); }
});
...
app.get("/v1/activity-photos/:id/download", async (req, res) => {
  try {
    ...
    if (!auth || !auth.studentId) return res.status(401).json({ ok: false, data: null, error: "Unauthorized" });
    const id = String(req.params.id || "").trim();
    const result = await query(`select p.*, a.status as album_status from activity_photos p join activity_albums a on a.id = p.album_id where p.id = $1 limit 1`, [id]);
    const photo = result.rows[0];
    const canManage = await canManageActivityAlbums_(auth.studentId);
    if (!photo || (photo.status !== "ready" && !canManage) || (photo.album_status === "archived" && !canManage)) return res.status(404).json({ ok: false, data: null, error: "Photo not found" });
    const url = await createSignedReadUrlForAttachment(photo, 300, { throwOnError: true, download: photo.original_name });
    return res.json({ ok: true, data: { url, expiresInSeconds: 300 }, error: null });
  } catch (error) { return res.status(500).json({ ok: false, data: null, error: error.message || "Internal error" }); }
});
...
app.post("/v1/activity-photos/:id/complete", async (req, res) => {
  try {
    ...
    if (!auth || !auth.studentId) return res.status(401).json({ ok: false, data: null, error: "Unauthorized" });
    const id = String(req.params.id || "").trim();
    const existing = await query(`select p.*, a.status as album_status from activity_photos p join activity_albums a on a.id = p.album_id
      where p.id = $1 and p.uploaded_by = $2 and p.status = 'pending' limit 1`, [id, auth.studentId]);
    ...
    const metadata = await getStorageObjectMetadata({ bucket: pending.bucket, storagePath: pending.storage_path });
    const storedMime = firstText(metadata && metadata.metadata && (metadata.metadata.mimetype || metadata.metadata.mimeType)).toLowerCase();
    const storedSize = Number(metadata && metadata.metadata && (metadata.metadata.size || metadata.metadata.contentLength));
    if (!metadata || !isActivityPhotoMime_(storedMime) || !Number.isFinite(storedSize) || storedSize <= 0 || storedSize > 15 * 1024 * 1024) {
      if (metadata) await removeStorageObject({ bucket: pending.bucket, storagePath: pending.storage_path });
      await query(`update activity_photos set status = 'deleted', deleted_at = now(), updated_at = now() where id = $1`, [id]);
      return res.status(400).json({ ok: false, data: null, error: "上傳檔案不符合照片格式或大小規範" });
    }
    const result = await query(`update activity_photos set status = 'ready', size_bytes = $3, mime_type = $4, updated_at = now() where id = $1 and uploaded_by = $2 and status = 'pending' returning *`, [id, auth.studentId, storedSize, storedMime]);
    if (!result.rows[0]) return res.status(404).json({ ok: false, data: null, error: "Upload not found" });
    return res.json({ ok: true, data: { photo: result.rows[0] }, error: null });
  } catch (error) { return res.status(500).json({ ok: false, data: null, error: error.message || "Internal error" }); }
});
...
app.patch("/v1/activity-photos/:id", async (req, res) => {
  try {
    ...
    const result = await query(`update activity_photos set status = $2, updated_at = now() where id = $1 and status <> 'deleted' returning *`, [String(req.params.id || ''), status]);
...
app.delete("/v1/activity-photos/:id", async (req, res) => {
  try {
    ...
    if (!(await canManageActivityAlbums_(auth.studentId))) return res.status(403).json({ ok: false, data: null, error: "Forbidden" });
    const id = String(req.params.id || "").trim();
    const result = await query(`select * from activity_photos where id = $1 and status <> 'deleted' limit 1`, [id]);
    const photo = result.rows[0];
    if (!photo) return res.status(404).json({ ok: false, data: null, error: "Photo not found" });
    await removeStorageObject({ bucket: photo.bucket, storagePath: photo.storage_path });
    await withTransaction(async ({ query: txQuery }) => {
      await txQuery(`update activity_photos set status = 'deleted', deleted_at = now(), updated_at = now() where id = $1`, [id]);
      await txQuery(`update activity_albums set cover_photo_id = null, updated_at = now() where cover_photo_id = $1`, [id]);
    });
    return res.json({ ok: true, data: { id }, error: null });
  } catch (error) { return res.status(500).json({ ok: false, data: null, error: error.message || "Internal error" }); }
});
