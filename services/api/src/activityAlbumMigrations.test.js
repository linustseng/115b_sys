...
test("031 reasserts private bucket/RLS after already-applied 029/030 without changing other buckets", async () => {
  ...
  assert.doesNotMatch(schema, /alter table storage\.objects enable row level security/i);
  assert.doesNotMatch(rls, /alter table storage\.objects enable row level security/i);
  ...
  assert.match(hardening, /storage_owner = current_user/i);
  assert.match(hardening, /not coalesce\(storage_rls_enabled, false\) or storage_policy_count <> 0/i);
  assert.match(hardening, /raise exception 'storage\.objects must be owner-hardened/i);
